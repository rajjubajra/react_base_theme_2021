...
export const reducerFetchMenu = (state = initalState, action) => {
  ...
  switch (action.type) {
    case actionTypes.START_FETCHING_MENU_MUSIC_THREE:
      return {
        fetching: true
      }
    /** FETCH DATA FOR Music menu page */
    case actionTypes.FETCHED_MENU_MUSIC_THREE:
      ...
      action.data.forEach(function (item) {
        arr.push({
          id: item.key,
          name: item.title,
          path: 'b4Music-three-' + item.uri.replace('base:', ''),
    ...
    case actionTypes.FETCH_ERROR_MENU_MUSIC_THREE:
      return {
        fetched: false,
        fetching: false,
        error: action.error
